...
from app.core.utils.models import Talent, TalentConstraint
...
class TalentService(CRUDBase[Talent, TalentCreate, TalentUpdate]):
    ...
    async def create_talent(self, db: Session,  data:TalentCreate) -> Talent:
        ...
        existing = db.query(Talent).filter(Talent.email == data.email).first()
        if existing:
            raise ValueError("Talent with this email already exists")
        talent = self.create(db, data)
        return talent
    
    async def update_talent(self, db: Session, talent_id:int, data: TalentUpdate):
        ...
        if data.contract_type == "full-time":
            data.hours = 40
        elif data.contract_type =="part-time":
            data.hours = 30
        elif data.contract_type == "student":
            data.hours = 24
    ...
    async def get_all_talents(self, db: Session,
                              name: str | None = None,
        ...
        if not query:
            raise ValueError("Talents not found")
        if tal_role:
